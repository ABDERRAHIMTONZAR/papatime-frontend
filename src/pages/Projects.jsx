...
import { FiPlus, FiTrash2, FiClock, FiChevronDown, FiChevronUp, FiUser } from 'react-icons/fi';
...
export default function Projects() {
  ...
  const createTache = async (projetId) => {
    if (!tacheForm.name.trim()) {
      toast.warning('Le nom est obligatoire !', '⚠️');
      return;
    }
    ...
    setTacheForm({ name: '', description: '', assignedId: '' });
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-8">
        ...
        {isAdmin && showForm && (
          <div className="bg-gray-900 rounded-2xl p-6 mb-6">
            ...
            <input
              ...
              className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg mb-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <input
              type="text"
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {isAdmin && projet.createdBy && (
                      <p className="text-gray-500 text-xs">Créé par {projet.createdBy.name || projet.createdBy.email}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => toggleProjet(projet.id)} className="text-gray-400 hover:text-white transition">
                    {expandedProjet === projet.id ? <FiChevronUp /> : <FiChevronDown />}
                  </button>
                  {isAdmin && (
                    <button onClick={() => deleteProjet(projet.id)} className="text-red-400 hover:text-red-300 transition">
                      <FiTrash2 />
                    </button>
                  )}
                </div>
                  ...
                  {isAdmin && showTacheForm === projet.id && (
                    <div className="bg-gray-800 rounded-lg p-4 mb-3">
                      <input
                        ...
                        className="w-full bg-gray-700 text-white px-3 py-2 rounded-lg mb-2 focus:outline-none text-sm focus:ring-2 focus:ring-indigo-500"
                      />
                      <input
                        type="text"
                        placeholder="Description (optionnel)"
                      ...
                      <select
                        ...
                        onChange={e => setTacheForm({...tacheForm, assignedId: e.target.value})}
                  ...
                  <div className="space-y-2">
                    {taches[projet.id]?.map(tache => (
                      <div key={tache.id} className="bg-gray-800 rounded-lg px-4 py-3 flex items-center justify-between">
                        <div>
                          <p className="text-white text-sm font-medium">{tache.name}</p>
                          {tache.assignedTo && (
                          ...
                          {!tache.assignedTo && !tache.equipe && (
                            <p className="text-gray-500 text-xs mt-1">Tous les membres</p>
                          )}
                        </div>
        ...
        {projets.length === 0 && (
          <p className="text-gray-400 text-center mt-12">
            {isAdmin ? 'Aucun projet — créez-en un !' : 'Aucun projet assigné pour le moment'}
          </p>
        )}
